...
export default function VideoCallPage() {
  ...
  useEffect(() => {
    ...
    const initRoom = async () => {
      try {
        setLoading(true);
        console.log('[VideoCall] Starting initRoom with bookingId:', bookingId);

        // Get room
        console.log('[VideoCall] Step 1: Getting room for booking...');
        const roomData = await videoCallService.getRoomForBooking(bookingId);
        console.log('[VideoCall] Step 1 SUCCESS - Room data:', roomData);
        setRoom(roomData);

        // Get session info
        console.log('[VideoCall] Step 2: Getting session info...');
        const sessionData = await videoCallService.getSessionInfo(bookingId);
        console.log('[VideoCall] Step 2 SUCCESS - Session info:', sessionData);
        setSessionInfo(sessionData);

        // Get local media
        console.log('[VideoCall] Step 3: Getting user media (camera/mic)...');
        const stream = await videoCallService.getUserMedia(true, true);
        console.log('[VideoCall] Step 3 SUCCESS - Got media stream:', stream.getTracks().map(t => t.kind));
        ...
        console.log('[VideoCall] All steps completed successfully!');
      } catch (err: any) {
        console.error('[VideoCall] ERROR in initRoom:', err);
        console.error('[VideoCall] Error response:', err.response?.data);
        console.error('[VideoCall] Error message:', err.message);
        setError(err.response?.data?.message || err.message || 'Failed to join call');
        toast.error(err.response?.data?.message || err.message || 'Failed to join call');
      } finally {
        setLoading(false);
      }
    };
  ...
  const remoteName = sessionInfo
    ? user?.id === sessionInfo.providerName
      ? sessionInfo.learnerName
      : sessionInfo.providerName
    : 'Participant';
      ...
      <header className="flex items-center justify-between px-4 py-3 bg-[#1a2332] border-b border-gray-700/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs">with {sessionInfo?.providerName || 'Provider'}</p>
          </div>
        ...
        <div className="w-full h-full rounded-xl overflow-hidden bg-[#243447] flex items-center justify-center">
          {remoteStream ? (
            ...
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm">Video feed from {remoteName}</p>
            </div>
          )}
        </div>
            ...
            {participants
              ...
              .map((p) => (
                <div key={p.userId} className="flex items-center gap-3 py-3">
                  ...
                  <span className="text-white text-sm">{p.userName || remoteName}</span>
                </div>
              ))}
          </div>
        </div>
