...
let UpdateSubscriptionPlanUseCase = class UpdateSubscriptionPlanUseCase {
    ...
    async execute(adminUserId, planId, dto) {
        ...
        if (dto.name || dto.price !== undefined || projectPosts !== undefined ||
            ...
            plan.updateDetails(dto.name ?? plan.name, dto.price ?? plan.price, projectPosts !== undefined ? projectPosts : plan.projectPosts, createCommunity !== undefined ? createCommunity : plan.createCommunity, (dto.badge ?? plan.badge), dto.color ?? plan.color);
        }
