...
import { IPinMessageUseCase } from './interfaces/IPinMessageUseCase';
@injectable()
export class PinMessageUseCase implements IPinMessageUseCase {
  constructor(
  ...
  public async execute(userId: string, messageId: string): Promise<CommunityMessage> {
    ...
    if (community.adminId !== userId) {
      throw new ForbiddenError('Only community admin can pin messages');
    }
    message.pin(userId);
